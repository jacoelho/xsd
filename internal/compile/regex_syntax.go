...
// RegexCategoryCache caches Go regexp category support checks during schema
// compilation.
type RegexCategoryCache map[string]bool

// ValidateXSDRegexSyntax validates XSD regex syntax and reports regexes that
// are valid XSD but not representable by Go regexp.
func ValidateXSDRegexSyntax(source string, categories RegexCategoryCache) error {
...
// CheckXSDRegexSyntax validates XSD regex syntax and reports whether the regex
// needs a matcher other than Go regexp.
func CheckXSDRegexSyntax(source string, categories RegexCategoryCache) (bool, error) {
	v := xsdRegexSyntaxValidator{categories: categories}
...
func (v *xsdRegexSyntaxValidator) consumeCategory(r rune) error {
	...
	switch {
	case v.categoryName == "":
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "invalid regex category escape")
...
func validGoRegexCategoryCached(categories RegexCategoryCache, name string) bool {
	if categories == nil {
		return validGoRegexCategory(name)
	}
	ok, found := categories[name]
	if found {
		return ok
	}
...
func (v *xsdRegexSyntaxValidator) validGoRegexCategory(name string) bool {
	return validGoRegexCategoryCached(v.categories, name)
}
...
func (v *xsdRegexSyntaxValidator) consumeQuantifier(r rune) error {
	...
	case r == ',':
		if v.quantifierSawComma || len(v.quantifierMin) == 0 {
			return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "invalid regex quantifier")
		}
...
func (v *xsdRegexSyntaxValidator) consumeEscaped(r rune) error {
	if !isXSDRegexEscape(r) {
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "invalid regex escape")
	}
...
func (v *xsdRegexSyntaxValidator) openNestedClass() error {
	if !v.classLastHyphen {
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "invalid nested regex character class")
	}
	v.unsupported = true
...
func (v *xsdRegexSyntaxValidator) closeClass() error {
	...
	if !class.hasTerm {
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "empty regex character class")
	}
...
func (v *xsdRegexSyntaxValidator) consumeAtom(r rune) error {
	...
	case '[':
		v.openClass()
	case ']':
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "unmatched regex character class end")
	case '(':
		v.groupDepth++
		v.canQuantify = false
		v.prevQuantifier = false
...
func (v *xsdRegexSyntaxValidator) closeGroup() error {
	if v.groupDepth == 0 {
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "unmatched regex group end")
	}
	v.groupDepth--
	v.canQuantify = true
	v.prevQuantifier = false
...
func (v *xsdRegexSyntaxValidator) finish() error {
	if v.escaped {
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "trailing regex escape")
	}
	if v.pendingCategory || v.inCategory {
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "unclosed regex category escape")
	}
	if v.inQuantifier {
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "unclosed regex quantifier")
	}
	if v.insideClass() {
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "unclosed regex character class")
	}
	if v.groupDepth != 0 {
		return xsderrors.SchemaCompile(xsderrors.CodeSchemaFacet, "unclosed regex group")
	}
	return nil
}
