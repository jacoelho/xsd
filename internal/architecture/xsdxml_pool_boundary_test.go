		...
		for _, decl := range parsed.Decls {
			...
			for _, spec := range genDecl.Specs {
				...
				if isSyncPoolType(valueSpec.Type) {
					t.Fatalf("xsdxml boundary violation: package-level sync.Pool variable in %s", file.relPath)
				}
				for _, value := range valueSpec.Values {
					...
					if isSyncPoolType(composite.Type) {
						t.Fatalf("xsdxml boundary violation: package-level sync.Pool composite in %s", file.relPath)
					}
				}
			}
		}
