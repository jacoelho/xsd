...
func TestTargetContextViewsRetainValidatedSchemaDefaults(t *testing.T) {
	t.Parallel()
	limits, err := NormalizeOptions(Options{})
	if err != nil {
		t.Fatal(err)
	}
	doc, err := parseSchemaDocument("common.xsd", "common.xsd", strings.NewReader(`<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" blockDefault="#all" finalDefault="#all" elementFormDefault="qualified" attributeFormDefault="qualified"/>`), limits, new(xmlstream.Reader))
	if err != nil {
		t.Fatal(err)
	}
	view := schemaSetDocument{doc: doc, effectiveTargetNS: "urn:adopted", adoptedTarget: true}
	if view.doc.defaults != doc.defaults {
		t.Fatalf("view defaults = %+v, want %+v", view.doc.defaults, doc.defaults)
	}
}

func TestTargetContextViewsShareSemanticSourceArena(t *testing.T) {
	t.Parallel()
	limits, err := NormalizeOptions(Options{})
	if err != nil {
		t.Fatal(err)
	}
	common := []byte(`<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"><xs:element name="shared" type="xs:string"/></xs:schema>`)
	resolver := source.Resolver(func(_, location string) (source.Source, error) {
		if location == "common.xsd" {
			return source.Bytes("common.xsd", common), nil
		}
	...
	work := newWorkBudget(dependencyWorkBudget, limits.MaxSchemaDependencySteps)
	graph, err := loadSchemaGraphOwned([]source.Source{root("a.xsd", "urn:a"), root("b.xsd", "urn:b")}, limits, &work)
	if err != nil {
		t.Fatal(err)
	}
	plan, err := graph.plan()
	...
	var commonViews []*schemaSetDocument
	for i := range plan.documents {
		if plan.documents[i].doc.name == "common.xsd" {
			commonViews = append(commonViews, &plan.documents[i])
		}
	}
	if len(commonViews) != 2 {
		t.Fatalf("common views = %d, want 2", len(commonViews))
	}
	if commonViews[0].doc != commonViews[1].doc {
		t.Fatal("chameleon views copied the semantic source document")
	}
	if commonViews[0].doc.root != commonViews[1].doc.root || len(commonViews[0].doc.arena) != 2 {
		t.Fatal("chameleon views do not share one source arena")
	}
	if commonViews[0].viewKey == commonViews[1].viewKey {
		t.Fatal("chameleon views lost effective-context identity")
	}
}
