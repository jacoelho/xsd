...
func TestParseWildcard(t *testing.T) {
	...
	tests := []struct {
		name       string
		attrs      WildcardAttrs
		wantMode   WildcardMode
		wantNS     []string
		wantOther  string
		wantProc   ProcessContents
		wantCode   xsderrors.Code
		wantNoName bool
	}{
		{
			name:       "defaults to any strict",
			wantMode:   WildcardAny,
			wantProc:   ProcessStrict,
			wantNoName: true,
		},
		{
		...
		},
		{
			...
			wantMode: WildcardTargetNamespace,
			wantNS:   []string{"urn:t"},
			wantProc: ProcessStrict,
		},
		{
...
func newWildcardNamespaceInterner() *wildcardNamespaceInterner {
	return &wildcardNamespaceInterner{
		ids: map[string]NamespaceID{"": EmptyNamespaceID},
	}
}

func (n *wildcardNamespaceInterner) InternNamespace(uri string) (NamespaceID, error) {
	if id, ok := n.ids[uri]; ok {
		return id, nil
	}
	next, err := checkedSchemaUint32(len(n.ids), "namespace limit exceeded")
	if err != nil {
		return 0, err
	}
	id := NamespaceID(next)
...
func (n *wildcardNamespaceInterner) mustID(uri string) NamespaceID {
	id, ok := n.ids[uri]
	if !ok {
