...
func TestCompileInheritedEnumerationRestrictionChain(t *testing.T) {
	...
	engine, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{
	...
	mustValidate(t, engine, `<root>0</root>`)
}
  ...
  <xs:annotation>
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(test.schema))})
...
func TestMissingIncludedSchemaLocationDoesNotInvalidateSchema(t *testing.T) {
	...
	_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(`
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
...
func TestOpaqueSourceMissingRelativeIncludeDoesNotInvalidateSchema(t *testing.T) {
	...
	for _, src := range sources {
		engine, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{src})
		if err != nil {
			t.Fatalf("Compile() error = %v", err)
		}
...
func TestMissingResolvedIncludeCleanupErrorIsNotSuppressed(t *testing.T) {
	...
	_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("root.xsd", []byte(`
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  <xs:include schemaLocation="optional.xsd"/>
</xs:schema>`)).WithResolver(resolver)})
...
func TestSchemaSetReferenceRules(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := xsdSchema.Compile(xsdSchema.Options{}, tt.sources())
...
func TestSchemaSetValidReferenceRules(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(tt.root)).WithResolver(resolver)}); err != nil {
				t.Fatalf("Compile() error = %v", err)
			}
		})
	}
...
func TestSchemaDirectivesMustPrecedeGlobalDeclarations(t *testing.T) {
  ...
  ` + directive + `
		...
		_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(schema)).WithResolver(resolver)})
...
func TestSchemaTopLevelOrderAcceptsLeadingDirectivesAndAnnotations(t *testing.T) {
  ...
  <xs:annotation/>
	...
	if _, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("root.xsd", []byte(root)).WithResolver(resolver)}); err != nil {
		t.Fatalf("Compile() error = %v", err)
	}
}
...
func TestInvalidSchemaDefaultsPrecedeResolverCalls(t *testing.T) {
	...
	for _, schema := range []string{
		...
		t.Run(schema, func(t *testing.T) {
			...
			_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("root.xsd", []byte(schema)).WithResolver(resolver)})
...
func TestInvalidChildSchemaDefaultsPrecedeDescendantResolverCalls(t *testing.T) {
	...
	_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("root.xsd", []byte(root)).WithResolver(resolver)})
	expectCode(t, err, xsderrors.CodeSchemaInvalidAttribute)
...
func TestUnsupportedLocalSchemaLocationsAreOptional(t *testing.T) {
	...
	engine, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("root.xsd", []byte(root))})
	if err != nil {
...
func TestArbitrarySourceIdentityIsNotParsedAsURIBase(t *testing.T) {
	for _, schema := range []string{
		...
		engine, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("x:%zz", []byte(schema))})
		if err != nil {
			t.Fatalf("Compile() error = %v", err)
		}
		mustValidate(t, engine, `<root/>`)
...
func TestUnixFileFallbackPreservesEffectiveBase(t *testing.T) {
	...
	t.Run("query replaced by relative path", func(t *testing.T) {
		...
		engine, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.File(rootPath)})
		if err != nil {
			t.Fatal(err)
		}
		mustValidate(t, engine, `<included/>`)
	})
	...
	t.Run("network authority does not become local path", func(t *testing.T) {
		...
		mustNotValidate(t, engine, `<included/>`, xsderrors.CodeValidationRoot)
	})
}
...
func TestMalformedFragmentSchemaLocationRemainsFatal(t *testing.T) {
	...
	_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("root.xsd", []byte(root))})
	expectCode(t, err, xsderrors.CodeSchemaReference)
}
...
func TestXMLNamespaceImportDoesNotCallResolver(t *testing.T) {
	...
	if _, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(schema)).WithResolver(resolver)}); err != nil {
		t.Fatalf("Compile() error = %v", err)
	}
	if called {
...
func TestResolvedIdentityUsesResolverReturnedSourceName(t *testing.T) {
	...
	mustValidate(t, engine, `<m:fromAlias xmlns:m="urn:main"/>`)
}
...
func TestResolverHandlesSchemaLocationBeforeGenericURIResolution(t *testing.T) {
	...
	mustValidate(t, engine, `<m:child xmlns:m="urn:main"/>`)
}
...
func TestCachedSourceIdentityKeepsDistinctFallbackContexts(t *testing.T) {
	...
	for _, sources := range [][]source.Source{
		...
		engine, err := xsdSchema.Compile(xsdSchema.Options{}, sources)
		if err != nil {
			t.Fatal(err)
		}
		mustValidate(t, engine, `<included/>`)
	...
	engine, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("root.xsd", root).WithResolver(resolver)})
	if err != nil {
		t.Fatal(err)
	}
	mustValidate(t, engine, `<included/>`)
...
func TestLateLoadedGenericReferenceContributesTargetContext(t *testing.T) {
	...
	mustValidate(t, engine, `<item xmlns="urn:a"/>`)
	mustValidate(t, engine, `<item xmlns="urn:b"/>`)
}
...
func TestSchemaLoaderCanonicalizesURIIdentityComponents(t *testing.T) {
	...
	for _, names := range [][2]string{
		...
		if _, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{
...
func TestCachedSchemaIdentityResolverContextCountsReferences(t *testing.T) {
	...
	if _, err := xsdSchema.Compile(xsdSchema.Options{MaxSchemaReferences: 4}, sources()); err != nil {
		t.Fatalf("Compile(exact references) error = %v", err)
	}
	_, err := xsdSchema.Compile(xsdSchema.Options{MaxSchemaReferences: 3}, sources())
...
func TestSchemaLoaderMatchesCanonicalURLIdentity(t *testing.T) {
	...
	mustValidate(t, engine, `<canonical/>`)
}
...
func TestFileSourceResolvesFromRelativePathContainingColon(t *testing.T) {
	...
	mustValidate(t, engine, `<relative/>`)
}
...
func TestEmptyIncludeLocationResolvesCurrentDocument(t *testing.T) {
	...
	engine, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(schema))})
	if err != nil {
		t.Fatalf("Compile() error = %v", err)
	}
	mustValidate(t, engine, `<root/>`)
...
func TestEmptyIncludeLocationResolvesXMLBaseWithFileSource(t *testing.T) {
	...
	mustValidate(t, engine, `<from-child/>`)
}
...
func TestQueryOnlyXMLBaseUsesRFC2396ForGenericIdentity(t *testing.T) {
	...
	mustValidate(t, engine, `<t:from-child xmlns:t="urn:test"/>`)
}
...
func TestResolvedSourceNameAndResolverErrorsRemainStructured(t *testing.T) {
  ...
  <xs:include schemaLocation="child.xsd"/>
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("root.xsd", []byte(root)).WithResolver(tt.resolver)})
...
func TestChameleonIncludesCloneTransitivelyForMultipleTargets(t *testing.T) {
  ...
  <xs:include schemaLocation="common.xsd"/>
	...
	engine, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{root("urn:b"), root("urn:a")})
	if err != nil {
		t.Fatalf("Compile() error = %v", err)
	}
	...
	mustNotValidate(t, engine, `<common/>`, xsderrors.CodeValidationRoot)
	mustNotValidate(t, engine, `<leaf/>`, xsderrors.CodeValidationRoot)
}
...
func TestExplicitChameleonSourceRetainsAbsentTargetContext(t *testing.T) {
  ...
  <xs:include schemaLocation="common.xsd"/>
	...
	engine, err := xsdSchema.Compile(xsdSchema.Options{MaxSchemaTargetContexts: 3}, sources())
	if err != nil {
		t.Fatalf("Compile() error = %v", err)
	}
	mustValidate(t, engine, `<shared>absent</shared>`)
	mustValidate(t, engine, `<shared xmlns="urn:named">named</shared>`)

	_, err = xsdSchema.Compile(xsdSchema.Options{MaxSchemaTargetContexts: 2}, sources())
	expectCode(t, err, xsderrors.CodeSchemaLimit)
}
...
func TestAbsentTargetIncludeCycleEstablishesRootContext(t *testing.T) {
  ...
  <xs:element name="b" type="xs:string"/>
	...
	mustValidate(t, engine, `<a>one</a>`)
	mustValidate(t, engine, `<b>two</b>`)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  <xs:element name="b"/>
	...
	mustValidate(t, engine, `<t:a xmlns:t="urn:test"/>`)
	mustValidate(t, engine, `<t:b xmlns:t="urn:test"/>`)
	mustValidate(t, engine, `<t:root xmlns:t="urn:test">ok</t:root>`)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  <xs:element name="dup"/>
	...
	mustValidate(t, engine, `<t:dup xmlns:t="urn:test"/>`)
}
...
func TestIPv6ZoneCaseKeepsSourceIdentitiesDistinct(t *testing.T) {
	...
	mustValidate(t, engine, `<a:a xmlns:a="urn:a"/>`)
	mustValidate(t, engine, `<b:b xmlns:b="urn:b"/>`)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" targetNamespace="urn:test">
  <xs:element name="root">
	...
	mustValidate(t, engine, `<t:root xmlns:t="urn:test"><item code="a"/><item code="b"/></t:root>`)
	mustNotValidate(t, engine, `<t:root xmlns:t="urn:test"><item code="a"/><item code="a"/></t:root>`, xsderrors.CodeValidationIdentity)
}
...
func TestCompileRejectsBuiltinTypeRedeclarations(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(schema))})
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:complexType name="Base">
	...
	mustValidate(t, engine, `<root><child><child><leaf>x</leaf></child><leaf>x</leaf></child></root>`)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="member" substitutionGroup="head"/>
	...
	mustValidate(t, engine, `<head><member/></head>`)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xs:element name="root">
	...
	mustNotValidate(t, engine, `<root xml:lang="@@"/>`, xsderrors.CodeValidationFacet)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="bad" type="absent"/>
	...
	mustNotValidate(t, engine, `<bad>1</bad>`, xsderrors.CodeValidationElement)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="anonymous" type="anonymousOuter"/>
	...
	mustValidate(t, engine, `<named>2</named>`)
	mustValidate(t, engine, `<anonymous>2</anonymous>`)
}
...
func TestUnionCanonicalizesRepeatedFlattenedMembers(t *testing.T) {
  ...
  <xs:element name="value" type="outer"/>
	...
	if got := len(build.SimpleTypes[outer].ValueSpec.Union); got != 3 {
		t.Fatalf("outer flattened members = %d, want 3 unique members", got)
	}
	engine := mustCompile(t, schema)
	mustValidate(t, engine, `<value>1</value>`)
	mustValidate(t, engine, `<value>true</value>`)
}
...
func TestSimpleUnionMemberEntryLimit(t *testing.T) {
  ...
  <xs:simpleType name="u2"><xs:union memberTypes="u1 xs:boolean"/></xs:simpleType>
	...
	if _, err := xsdSchema.Compile(xsdSchema.Options{MaxSimpleUnionMemberEntries: 6}, sourceFor()); err != nil {
		t.Fatalf("Compile(exact union member entries) error = %v", err)
	}
	_, err := xsdSchema.Compile(xsdSchema.Options{MaxSimpleUnionMemberEntries: 5}, sourceFor())
	expectCode(t, err, xsderrors.CodeSchemaLimit)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="good" type="xs:int"/>
	...
	for _, doc := range []string{`<union>x</union>`, `<list>x</list>`, `<content>x</content>`} {
		mustNotValidate(t, engine, doc, xsderrors.CodeValidationElement)
	}
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="good" type="xs:int"/>
	...
	mustValidate(t, engine, `<good>1</good>`)
	mustNotValidate(t, engine, `<bad>value</bad>`, xsderrors.CodeValidationElement)
	mustNotValidate(t, engine, `<bad xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:nil="true"/>`, xsderrors.CodeValidationElement)
	mustNotValidate(t, engine, `<badComplex>value</badComplex>`, xsderrors.CodeValidationElement)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="transitive" type="badList" fixed="value"/>
	...
	mustNotValidate(t, engine, `<direct/>`, xsderrors.CodeValidationElement)
	mustNotValidate(t, engine, `<transitive>value</transitive>`, xsderrors.CodeValidationElement)
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="skip"><xs:complexType><xs:anyAttribute processContents="skip"/></xs:complexType></xs:element>
	...
	for _, doc := range []string{
		...
		mustNotValidate(t, engine, doc, xsderrors.CodeValidationAttribute)
	}
	mustValidate(t, engine, `<skip globalBad="value"/>`)
}
...
func TestAnonymousSimpleTypeCannotHaveName(t *testing.T) {
	_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(`<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"><xs:simpleType name="parent"><xs:restriction><xs:simpleType name="child"><xs:restriction base="xs:string"/></xs:simpleType></xs:restriction></xs:simpleType></xs:schema>`))})
	expectCode(t, err, xsderrors.CodeSchemaInvalidAttribute)
}
...
func TestSimpleContentRestrictionSimpleTypeMustPrecedeFacets(t *testing.T) {
  ...
  <xs:element name="root"><xs:complexType><xs:simpleContent><xs:restriction base="Base">`
	...
	if _, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("ordered.xsd", []byte(prefix+inline+`<xs:minLength value="1"/>`+suffix))}); err != nil {
		t.Fatalf("Compile(ordered) error = %v", err)
	}
	_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("reversed.xsd", []byte(prefix+`<xs:minLength value="1"/>`+inline+suffix))})
	expectCode(t, err, xsderrors.CodeSchemaContentModel)
}
...
func TestRestrictionElementPropertiesCannotBeLoosened(t *testing.T) {
	...
	for _, body := range tests {
		_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(`<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">`+body+`</xs:schema>`))})
		expectCode(t, err, xsderrors.CodeSchemaContentModel)
	}
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="member" substitutionGroup="head"/>
	...
	mustNotValidate(t, engine, `<member>x</member>`, xsderrors.CodeValidationFacet)
}
...
func TestSchemaAdmissionRejectsForeignElementsOutsideAnnotationPayload(t *testing.T) {
	_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("foreign.xsd", []byte(`
...
func TestSchemaAdmissionTreatsAnnotationPayloadAsOpaque(t *testing.T) {
  ...
  <xs:element name="root"/>
	...
	engine, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("opaque.xsd", []byte(schema))})
	if err != nil {
		t.Fatalf("Compile() error = %v", err)
	}
	mustValidate(t, engine, `<root/>`)
...
func TestSchemaAdmissionRejectsSchemaAttributesOnAnnotationEnvelopes(t *testing.T) {
	for _, envelope := range []string{"appinfo", "documentation"} {
		t.Run(envelope, func(t *testing.T) {
			...
			_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes(envelope+".xsd", []byte(schema))})
			expectCode(t, err, xsderrors.CodeSchemaInvalidAttribute)
		})
	}
}
...
func TestSchemaAdmissionRejectsDirectiveChildren(t *testing.T) {
	for _, directive := range []string{"include", "import"} {
		t.Run(directive, func(t *testing.T) {
			...
			_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes(directive+".xsd", []byte(schema))})
			expectCode(t, err, xsderrors.CodeSchemaContentModel)
		})
	}
}
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="root">
	...
	mustNotValidate(t, engine, `<root><blockedMember>1</blockedMember></root>`, xsderrors.CodeValidationElement)
}
...
func TestUnsupportedFeaturesAreExplicit(t *testing.T) {
	_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(`<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"><xs:redefine schemaLocation="a.xsd"/></xs:schema>`))})
	expectCode(t, err, xsderrors.CodeUnsupportedRedefine)

	_, err = xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(`<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"><xs:element name="r"><xs:complexType><xs:anyAttribute notQName="##defined"/></xs:complexType></xs:element></xs:schema>`))})
...
func TestCompileOptionsSchemaXMLLimits(t *testing.T) {
	...
	_, err := xsdSchema.Compile(xsdSchema.Options{MaxSchemaDepth: 2}, []source.Source{source.Bytes("schema.xsd", []byte(deepSchema))})
	expectCategoryCode(t, err, xsderrors.CategorySchemaParse, xsderrors.CodeSchemaLimit)
	if _, boundaryErr := xsdSchema.Compile(xsdSchema.Options{MaxSchemaDepth: 3}, []source.Source{source.Bytes("schema.xsd", []byte(deepSchema))}); boundaryErr != nil {
	...
	_, err = xsdSchema.Compile(xsdSchema.Options{MaxSchemaAttributes: 1}, []source.Source{source.Bytes("schema.xsd", []byte(attrSchema))})
	expectCategoryCode(t, err, xsderrors.CategorySchemaParse, xsderrors.CodeSchemaLimit)
	if _, boundaryErr := xsdSchema.Compile(xsdSchema.Options{MaxSchemaAttributes: 2}, []source.Source{source.Bytes("schema.xsd", []byte(attrSchema))}); boundaryErr != nil {
	...
	_, err = xsdSchema.Compile(xsdSchema.Options{MaxSchemaTokenBytes: 128}, []source.Source{source.Bytes("schema.xsd", []byte(textSchema))})
	expectCategoryCode(t, err, xsderrors.CategorySchemaParse, xsderrors.CodeSchemaLimit)
	...
	_, err = xsdSchema.Compile(xsdSchema.Options{MaxSchemaInstantiatedNodes: 3}, []source.Source{source.Bytes("schema.xsd", []byte(nodeSchema))})
	expectCategoryCode(t, err, xsderrors.CategorySchemaParse, xsderrors.CodeSchemaLimit)
}

func TestSchemaParserDoesNotRetainOpaqueAnnotationPayload(t *testing.T) {
	limits, err := xsdSchema.NormalizeOptions(xsdSchema.Options{})
	if err != nil {
		t.Fatal(err)
	}
	root, err := xsdSchema.ParseSchemaRootForTest([]byte(`<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"><xs:annotation><xs:appinfo>text<payload attr="value"><nested/></payload></xs:appinfo></xs:annotation></xs:schema>`), limits)
	if err != nil {
		t.Fatal(err)
	}
	appinfo := root.ChildForTest(0).ChildForTest(0)
	if appinfo.HasNonWhitespaceText() || len(appinfo.ChildrenForTest()) != 0 {
		t.Fatalf("retained opaque payload: text=%t children=%d", appinfo.HasNonWhitespaceText(), len(appinfo.ChildrenForTest()))
	}
}
...
func TestCompileOptionsSchemaSourceByteLimit(t *testing.T) {
	...
	if _, err := xsdSchema.Compile(xsdSchema.Options{MaxSchemaSourceBytes: int64(len(schema))}, []source.Source{source.Bytes("schema.xsd", []byte(schema))}); err != nil {
		t.Fatalf("Compile() source byte boundary error = %v", err)
	}

	_, err := xsdSchema.Compile(xsdSchema.Options{MaxSchemaSourceBytes: int64(len(schema) - 1)}, []source.Source{source.Bytes("schema.xsd", []byte(schema))})
...
func TestSchemaNamespaceContextsAreIsolated(t *testing.T) {
	...
	root, err := xsdSchema.ParseSchemaRootForTest([]byte(`
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"
           xmlns:t="urn:test"
           xmlns="urn:test"
           targetNamespace="urn:test">
  ...
  <xs:element name="defaulted" type="base" default="value"/>
  <xs:element name="local" xmlns:u="urn:test" type="u:base" fixed="u:value"/>
</xs:schema>`), limits)
	if err != nil {
		t.Fatalf("parse() error = %v", err)
	}
	if _, ok := root.NamespaceLookupForTest("t"); ok {
		t.Fatal("root namespace context should not be retained")
	}
	annotation := root.ChildForTest(1)
	if _, ok := annotation.NamespaceLookupForTest("t"); ok {
		t.Fatal("annotation namespace context should not be retained")
	}
	prefixed := root.ChildForTest(2)
	if _, ok := prefixed.NamespaceLookupForTest("t"); ok {
		t.Fatal("ordinary element namespace context should not be retained")
	}
	defaulted := root.ChildForTest(3)
	if got, ok := defaulted.NamespaceLookupForTest(""); !ok || got != "urn:test" {
		t.Fatalf("default value namespace = %q, %t; want urn:test, true", got, ok)
	}
	local := root.ChildForTest(4)
	if got, ok := local.NamespaceLookupForTest("u"); !ok || got != "urn:test" {
		t.Fatalf("fixed value namespace = %q, %t; want urn:test, true", got, ok)
	}
}
...
func TestCompileOptionsRejectNegativeLimits(t *testing.T) {
	...
	for _, opts := range tests {
		_, err := xsdSchema.Compile(opts, []source.Source{schemaSource})
		expectCategoryCode(t, err, xsderrors.CategorySchemaCompile, xsderrors.CodeSchemaLimit)
	}
}
...
func TestRuntimeKeyRefAmbiguousSiblingKeysWithSameDisplayPathDoesNotResolve(t *testing.T) {
  ...
  <xs:element name="root">
	...
	engine := mustCompile(t, schema)
	mustNotValidate(t, engine, `<root rid="1"><group id="1"/><group id="1"/></root>`, xsderrors.CodeValidationIdentity)
}
...
func TestFreezeRuntimeConsumesCompilerRuntime(t *testing.T) {
  ...
  <xs:element name="r">
	...
	engine := rt
	mustValidate(t, engine, `<r code="US"><child>x</child></r>`)
...
func TestFreezeRuntimeClearsCompilerMutationAliases(t *testing.T) {
  ...
  <xs:element name="r">
	...
	frozen, err := xsdSchema.FreezeCompilerRuntimeForTest(c)
	if err != nil {
...
func TestPublishedSchemaOwnsValidationStorage(t *testing.T) {
  ...
  <xs:element name="root">
	...
	aliases.GlobalTypes[mustQName(t, &aliases.Names, "Code")] = xsdSchema.ComplexRef(itemType)
	aliases.SimpleTypes[codeID].ValueSpec.Union[0] = xsdSchema.NoSimpleType
	aliases.Elements[rootID].Identity[0] = xsdSchema.NoIdentityConstraint
	...
	interner := xsdSchema.NewNameInterner(&aliases.Names)
	...
	if member, ok := aliases.Substitutions.MemberByName(headID, itemName); !ok || member == xsdSchema.NoElement {
		t.Fatal("test schema did not compile a substitution lookup")
	}
	mustValidate(t, engine, `<root><item id="one" code="US"/><h/></root>`)
	mustNotValidate(t, engine, `<root><item id="one" code="CA"/></root>`, xsderrors.CodeValidationAttribute)
	mustNotValidate(t, engine, `<root><item id="same" code="US"/><item id="same" code="US"/></root>`, xsderrors.CodeValidationIdentity)
}
...
func TestFreezeRuntimeKeepsCompilerStateOnValidationFailure(t *testing.T) {
	...
	build.GlobalElements[rootName] = xsdSchema.NoElement

	_, err := xsdSchema.FreezeCompilerRuntimeForTest(c)
	if err == nil {
...
func frozenCompilerRuntime(t *testing.T, schema string) (*xsdSchema.Compiler, *xsdSchema.Schema) {
	t.Helper()
	c := compiledCompilerRuntime(t, schema)
	frozen, err := xsdSchema.FreezeCompilerRuntimeForTest(c)
	if err != nil {
...
func validationRuntimeForTest(tb testing.TB, rt *xsdSchema.Schema) *xsdSchema.Schema {
	tb.Helper()
	if rt != nil {
		return rt
	}
...
func compiledCompilerRuntime(t *testing.T, schema string) *xsdSchema.Compiler {
	t.Helper()
	limits, err := xsdSchema.NormalizeOptions(xsdSchema.Options{})
	if err != nil {
		t.Fatal(err)
	}
	c, err := xsdSchema.NewCompilerForTest(limits)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSimpleContentFacetRestrictionValidatesBounds(t *testing.T) {
	const schema = `<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  <xs:complexType name="Base">
    <xs:simpleContent>
  ...
  <xs:element name="root">
	...
	engine := mustCompile(t, schema)
	mustValidate(t, engine, `<root>10</root>`)
	mustNotValidate(t, engine, `<root>11</root>`, xsderrors.CodeValidationFacet)
}
...
func TestSimpleContentRestrictionAllowsEmptiableMixedBaseWithInlineType(t *testing.T) {
  ...
  <xs:element name="root">
	...
	mustValidate(t, engine, `<root>value</root>`)
}

type qnameLookup interface {
	LookupQName(namespace, local string) (xsdSchema.QName, bool)
}
...
func mustQName(t *testing.T, rt qnameLookup, local string) xsdSchema.QName {
	t.Helper()
...
func TestMixedSimpleContentExtensionChain(t *testing.T) {
  ...
  <xs:element name="root" type="B"/>
	...
	_, err := xsdSchema.Compile(xsdSchema.Options{}, []source.Source{source.Bytes("schema.xsd", []byte(nonMixedBase))})
	expectCode(t, err, xsderrors.CodeSchemaContentModel)
}
...
func TestRuntimeElementAccessor(t *testing.T) {
	...
	if _, ok := rt.Element(xsdSchema.NoElement); ok {
		t.Error("element(noElement) resolved, want miss")
	}
	if _, ok := rt.Element(xsdSchema.ElementID(1 << 30)); ok {
		t.Error("element(out of range) resolved, want miss")
	}
	rootName := mustQName(t, rt, "root")
	_, rootInfo, ok := rt.RootElement(xsdSchema.RuntimeName{Known: true, Name: rootName})
