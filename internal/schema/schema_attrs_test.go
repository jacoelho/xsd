...
func testRawNodeWithLocalAttrs(local string, attrs []string) *schemaSyntaxNode {
...
func TestCheckReferenceAttributes(t *testing.T) {
	...
	tests := []struct {
		name        string
		validate    func(*schemaSyntaxNode) error
		local       string
		attrs       []string
		wantMessage string
	}{
		{
			...
			validate: checkElementRefAttributesSyntax,
			local:    elementChild,
		...
		},
		{
			...
			validate: checkAttributeRefAttributesSyntax,
		...
		},
		{
			...
			validate: checkGroupOccurrenceAttributesSyntax,
			local:    groupChild,
			attrs:    []string{"id", "ref", "minOccurs", "maxOccurs"},
		},
		{
...
func TestCheckRawSchemaAttributes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := checkSchemaAttributesSyntax(testRawNodeWithLocalAttrs(tt.element, tt.attrs))
			if tt.wantMessage != "" {
				expectInvalidAttributeMessage(t, err, tt.wantMessage)
				return
			}
			if err != nil {
				t.Fatalf("checkSchemaAttributesSyntax() error = %v", err)
			}
		})
	}
}
...
func TestCheckLocalElementAttributes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := checkLocalElementAttributesSyntax(testRawNodeWithLocalAttrs(elementChild, tt.attrs))
			if tt.wantMessage != "" {
				expectInvalidAttributeMessage(t, err, tt.wantMessage)
				return
			}
			if err != nil {
				t.Fatalf("checkLocalElementAttributesSyntax() error = %v", err)
			}
		})
	}
}
...
func TestCheckLocalTypeAttributes(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			validate: checkLocalSimpleTypeAttributesSyntax,
		...
		},
		{
			...
			validate: checkLocalComplexTypeAttributesSyntax,
