...
func TestTemporalSubkind(t *testing.T) {
	tests := []struct {
		...
		{kind: value.KindDateTime, subkind: 0, ok: true},
		{kind: value.KindDate, subkind: 1, ok: true},
		{kind: value.KindTime, subkind: 2, ok: true},
		{kind: value.KindGYearMonth, subkind: 3, ok: true},
		{kind: value.KindGYear, subkind: 4, ok: true},
		{kind: value.KindGMonthDay, subkind: 5, ok: true},
		{kind: value.KindGDay, subkind: 6, ok: true},
		{kind: value.KindGMonth, subkind: 7, ok: true},
		{kind: value.KindInvalid, ok: false},
	}
...
func TestTemporalKeyBytesMatchesTemporalEqual(t *testing.T) {
	cases := []struct {
		...
		{kind: value.KindTime, a: "00:30:00+02:00", b: "22:30:00Z"},
		{kind: value.KindTime, a: "23:59:60+02:00", b: "22:00:00Z"},
		{kind: value.KindTime, a: "23:59:60Z", b: "00:00:00Z"},
		{kind: value.KindDateTime, a: "2000-01-01T00:30:00+02:00", b: "1999-12-31T22:30:00Z"},
		{kind: value.KindDate, a: "2002-10-10+05:00", b: "2002-10-09Z"},
		{kind: value.KindGYearMonth, a: "2002-10+05:00", b: "2002-09Z"},
	}

	for _, tc := range cases {
		a, err := value.Parse(tc.kind, []byte(tc.a))
		if err != nil {
			t.Fatalf("parse %q: %v", tc.a, err)
		}
		b, err := value.Parse(tc.kind, []byte(tc.b))
		if err != nil {
...
func TestDurationKeyBytes(t *testing.T) {
	...
	dur = value.Duration{Years: 1, Months: 2}
	key = DurationKeyBytes(nil, dur)
	want = []byte{1}
	...
	dur = value.Duration{Seconds: secDec}
	key = DurationKeyBytes(nil, dur)
	want = []byte{1}
	want = num.EncodeDecKey(want, num.IntZero.AsDec())
	...
	dur = value.Duration{Negative: true}
	key = DurationKeyBytes(nil, dur)
...
func TestDurationKeyBytesPrecision(t *testing.T) {
	...
	left := value.Duration{Seconds: leftSec}
	right := value.Duration{Seconds: rightSec}
...
func TestDurationKeyBytesAvoidsOverflow(t *testing.T) {
	...
	if int64(years) > math.MaxInt64/12 {
		tested = true
		dur := value.Duration{Years: years}
...
func durationKeyBytesLegacy(dst []byte, dur value.Duration) []byte {
...
func legacyDurationSecondsTotal(dur value.Duration) num.Dec {
	total := dur.Seconds
