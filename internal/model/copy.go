...
func copyAnyElement(elem *AnyElement, opts CopyOptions) *AnyElement {
	...
	clone := *elem
	if !opts.PreserveSourceNamespace && clone.TargetNamespace == "" && opts.SourceNamespace != "" {
...
func isChameleonRemap(opts CopyOptions) bool {
	if opts.RemapQName == nil || opts.SourceNamespace == "" {
		return false
	}
	remapped := opts.RemapQName(QName{Local: "x"})
	return remapped.Namespace == opts.SourceNamespace && remapped.Namespace != ""
}
...
func copyIdentityConstraints(constraints []*IdentityConstraint, opts CopyOptions) []*IdentityConstraint {
	...
	for _, constraint := range constraints {
		...
		if !constraint.ReferQName.IsZero() && constraint.ReferQName.Namespace == "" {
