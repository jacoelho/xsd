...
func TestMinInclusive(t *testing.T) {
	baseType := &types.SimpleType{
		QName: types.QName{Local: "integer"},
		Restriction: &types.Restriction{
			Base: types.QName{Local: "integer"},
		},
	}
	...
	for _, tt := range tests {
		intVal, err := lexicalparser.ParseInteger(tt.value)
		if err != nil {
			t.Fatalf("ParseInteger(%q) error = %v", tt.value, err)
		}
		tv := types.NewIntegerValue(types.NewParsedValue(tt.value, intVal), baseType)
...
func TestTotalDigits(t *testing.T) {
	baseType := &types.SimpleType{
		QName: types.QName{Local: "decimal"},
		Restriction: &types.Restriction{
			Base: types.QName{Local: "decimal"},
		},
	}
	baseType.MarkBuiltin()
...
func TestLengthCalculation(t *testing.T) {
	tests := []struct {
		...
		// base64Binary - length in octets (decoded bytes)
		{
			...
			baseType:   types.GetBuiltin(types.TypeNameBase64Binary),
			wantLength: 1,
		},
		{
...
func TestLengthFacetsWithHexBinary(t *testing.T) {
	hexBinaryType := types.GetBuiltin(types.TypeNameHexBinary)
...
func TestLengthFacetsWithBase64Binary(t *testing.T) {
	base64BinaryType := types.GetBuiltin(types.TypeNameBase64Binary)
...
func TestLengthFacetsWithListType(t *testing.T) {
	itemType := types.GetBuiltin(types.TypeNameInteger)
	listType := &types.SimpleType{
		QName: types.QName{Local: "integerList"},
		List:  &types.ListType{ItemType: itemType.Name()},
	}
...
// Per XSD 1.0 errata, length, minLength, and maxLength facets are ignored for QName and NOTATION types.
...
func TestLengthFacetsWithQNameRestriction(t *testing.T) {
	...
	restrictedType.SetVariety(types.AtomicVariety)
