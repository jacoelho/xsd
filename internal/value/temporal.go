...
func Parse(kind Kind, lexical []byte) (Value, error) {
	...
	tzKind := TimezoneKindFromLexical(trimmed)
	...
	case KindGYearMonth:
		parsed, err := ParseGYearMonth(trimmed)
		if err != nil {
			return Value{}, err
		}
		return Value{Kind: kind, Time: parsed, TimezoneKind: tzKind}, nil
	case KindGYear:
		parsed, err := ParseGYear(trimmed)
		if err != nil {
			return Value{}, err
		}
		return Value{Kind: kind, Time: parsed, TimezoneKind: tzKind}, nil
	case KindGMonthDay:
		parsed, err := ParseGMonthDay(trimmed)
		if err != nil {
			return Value{}, err
		}
		return Value{Kind: kind, Time: parsed, TimezoneKind: tzKind}, nil
	case KindGDay:
		parsed, err := ParseGDay(trimmed)
		if err != nil {
			return Value{}, err
		}
		return Value{Kind: kind, Time: parsed, TimezoneKind: tzKind}, nil
	case KindGMonth:
		parsed, err := ParseGMonth(trimmed)
		if err != nil {
			return Value{}, err
		}
		return Value{Kind: kind, Time: parsed, TimezoneKind: tzKind}, nil
...
func Canonical(v Value) string {
	...
	return CanonicalDateTimeString(v.Time, v.Kind.String(), v.TimezoneKind)
}
...
func hasDateTimeLeapSecond(lexical []byte) bool {
	main, _ := SplitTimezone(string(lexical))
	_, after, ok := strings.Cut(main, "T")
	if !ok {
		return false
	}
	timePart := after
	hour, minute, second, _, ok := ParseTimeParts(timePart)
	if !ok {
		return false
	}
...
func hasTimeLeapSecond(lexical []byte) bool {
	main, _ := SplitTimezone(string(lexical))
	hour, minute, second, _, ok := ParseTimeParts(main)
	if !ok {
		return false
	}
	return hour == 23 && minute == 59 && second == 60
