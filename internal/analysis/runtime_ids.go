...
type RuntimeIDPlan struct {
	...
	TypeIDs          map[TypeID]runtime.TypeID
	ElementIDs       map[ElemID]runtime.ElemID
	AttributeIDs     map[AttrID]runtime.AttrID
	BuiltinTypeNames []model.TypeName
}

// BuildRuntimeIDPlan constructs a deterministic runtime ID assignment plan.
func BuildRuntimeIDPlan(registry *Registry) (*RuntimeIDPlan, error) {
	if registry == nil {
	...
	plan := &RuntimeIDPlan{
		...
		TypeIDs:          make(map[TypeID]runtime.TypeID, len(registry.TypeOrder)),
		ElementIDs:       make(map[ElemID]runtime.ElemID, len(registry.ElementOrder)),
		AttributeIDs:     make(map[AttrID]runtime.AttrID, len(registry.AttributeOrder)),
	}
...
func BuiltinTypeNames() []model.TypeName {
	return slices.Clone(builtinTypeNames())
}
