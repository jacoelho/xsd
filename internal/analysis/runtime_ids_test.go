...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xs:element name="root" type="tns:Derived"/>
	...
	plan1, err := analysis.BuildRuntimeIDPlan(reg1)
	if err != nil {
		t.Fatalf("analysis.BuildRuntimeIDPlan(reg1) error = %v", err)
	}
	plan2, err := analysis.BuildRuntimeIDPlan(reg2)
	if err != nil {
		t.Fatalf("analysis.BuildRuntimeIDPlan(reg2) error = %v", err)
	}
	...
	assertTypeOrder := func(plan *analysis.RuntimeIDPlan, reg *analysis.Registry) {
	...
	assertElementOrder := func(plan *analysis.RuntimeIDPlan, reg *analysis.Registry) {
	...
	assertAttributeOrder := func(plan *analysis.RuntimeIDPlan, reg *analysis.Registry) {
