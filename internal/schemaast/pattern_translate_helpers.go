...
var outsideMetaHandlers = map[byte]outsideMetaHandler{
	...
	'*': func(t *patternTranslator) error { return t.emitOutsideQuantifier('*') },
	'+': func(t *patternTranslator) error { return t.emitOutsideQuantifier('+') },
	'?': func(t *patternTranslator) error { return t.emitOutsideQuantifier('?') },
...
func (t *patternTranslator) checkLazyQuantifier() error {
	...
	if isQuantifierStart(t.pattern[t.i]) {
		start := max(t.i-2, 0)
		end := min(t.i+1, len(t.pattern))
		return fmt.Errorf("pattern-syntax-error: quantifier follows quantifier in %q", t.pattern[start:end])
	}
	t.justWroteQuantifier = false
	return nil
}

func isQuantifierStart(ch byte) bool {
	return ch == '*' || ch == '+' || ch == '?' || ch == '{'
}
...
func (t *patternTranslator) handleRepeatQuantifier() (bool, error) {
	...
	if !t.canWriteQuantifier() {
		return true, fmt.Errorf("pattern-syntax-error: repeat quantifier has no target")
	}
...
func (t *patternTranslator) emitOutsideQuantifier(ch byte) error {
	if !t.canWriteQuantifier() {
		return fmt.Errorf("pattern-syntax-error: quantifier %q has no target", string(ch))
	}
...
func (t *patternTranslator) canWriteQuantifier() bool {
	if t.result.Len() == 0 {
		return false
	}
	...
	case '(', '|':
		return len(result) >= 2 && result[len(result)-2] == '\\'
	default:
		return true
	}
}
