...
func (t *patternTranslator) handleEscape() (bool, error) {
	...
	if t.inCharClass() && t.classState.lastWasDash && isNonCharacterClassEscape(nextChar) {
		return true, fmt.Errorf("pattern-syntax-error: '-' cannot form a range with non-character escape \\%c in %q", nextChar, t.pattern)
	}
	if nextChar == 'u' {
...
func isNonCharacterClassEscape(nextChar byte) bool {
	switch nextChar {
	case 'i', 'I', 'c', 'C', 'd', 'D', 's', 'S', 'w', 'W', 'p', 'P':
		return true
	default:
		return false
	}
