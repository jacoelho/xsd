...
func TestParse(t *testing.T) {
	...
	doc, err := parseDocument(strings.NewReader(xmlData))
	if err != nil {
...
func TestParseNamespaceDeclarations(t *testing.T) {
	...
	foundPrefix := false
	for _, attr := range attrs {
		if attr.NamespaceURI() != value.XMLNSNamespace {
			continue
		}
...
func TestParseRejectsReservedNamespacePrefixes(t *testing.T) {
	...
	for _, xmlData := range cases {
		if _, err := parseDocument(strings.NewReader(xmlData)); err == nil {
...
func TestParseNamespaceUndeclareRedeclare(t *testing.T) {
	...
	for _, attr := range doc.Attributes(root) {
		if attr.NamespaceURI() == value.XMLNSNamespace && attr.LocalName() == "xmlns" {
...
func TestDocumentOutOfBoundsNodeID(t *testing.T) {
	doc, err := parseDocument(strings.NewReader(`<root><child/></root>`))
	if err != nil {
		t.Fatalf("parseDocument() error = %v", err)
	}
...
func TestParseRejectsNonXMLWhitespaceOutsideRoot(t *testing.T) {
	xmlData := "\u00a0<root/>"
	if _, err := parseDocument(strings.NewReader(xmlData)); err == nil {
		t.Fatal("parseDocument() should reject non-XML whitespace outside root")
	}
}
...
func TestParseRejectsBOMAfterRoot(t *testing.T) {
	xmlData := "<root/>\uFEFF"
	if _, err := parseDocument(strings.NewReader(xmlData)); err == nil {
		t.Fatal("parseDocument() should reject BOM outside the document start")
	}
}
...
func TestIsIgnorableOutsideRoot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := value.IsIgnorableOutsideRoot(tt.data, tt.allowBOM); got != tt.want {
