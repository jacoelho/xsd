...
// compileXPathPrograms compiles a restricted XPath expression into runtime path programs.
func compileXPathPrograms(expr string, nsContext map[string]string, policy xsdpath.AttributePolicy, schema *runtime.Schema) ([]runtime.PathProgram, error) {
	if schema == nil {
		return nil, xpathErrorf("schema is nil")
	}
	parsed, err := xsdpath.Parse(expr, nsContext, policy)
	if err != nil {
		return nil, err
	}
	return compileXPathExpression(parsed, schema)
}

func compileXPathExpression(parsed xsdpath.Expression, schema *runtime.Schema) ([]runtime.PathProgram, error) {
	if schema == nil {
		return nil, xpathErrorf("schema is nil")
	}
...
func compilePathProgram(path xsdpath.Path, schema *runtime.Schema) (runtime.PathProgram, error) {
	...
	for _, step := range path.Steps {
		...
		if step.Axis == xsdpath.AxisSelf && step.Test.Any {
			if len(ops) == 0 {
		...
		if step.Axis == xsdpath.AxisDescendant || step.Axis == xsdpath.AxisDescendantOrSelf {
			return runtime.PathProgram{}, xpathErrorf("xpath uses unsupported descendant axis")
		}
		if step.Axis != xsdpath.AxisChild {
...
func isDescendStep(step xsdpath.Step) bool {
	return step.Axis == xsdpath.AxisDescendantOrSelf && step.Test.Any
}

func compileNodeTest(test xsdpath.NodeTest, schema *runtime.Schema, attribute bool) (runtime.PathOp, error) {
...
func xpathErrorf(format string, args ...any) error {
	...
	params := append([]any{xsdpath.ErrInvalidXPath}, args...)
