...
func TestSimpleTypeValidateAppliesFacets(t *testing.T) {
	base := types.GetBuiltin(types.TypeNameInteger)
	if base == nil {
		t.Fatalf("expected builtin integer")
	}
	maxFacet, err := types.NewMaxInclusive("10", base)
	if err != nil {
		t.Fatalf("NewMaxInclusive: %v", err)
	}
	st, err := types.NewAtomicSimpleType(types.QName{Namespace: "urn:test", Local: "Int10"}, "urn:test", &types.Restriction{
		Base:   types.QName{Namespace: types.XSDNamespace, Local: "integer"},
		Facets: []any{maxFacet},
	})
	if err != nil {
...
func TestSimpleTypeValidateUnionMembers(t *testing.T) {
	...
	int10, err := types.NewAtomicSimpleType(types.QName{Namespace: "urn:test", Local: "Int10"}, "urn:test", &types.Restriction{
		Base:   types.QName{Namespace: types.XSDNamespace, Local: "integer"},
		Facets: []any{maxFacet},
	})
	if err != nil {
	...
	union, err := types.NewUnionSimpleType(types.QName{Namespace: "urn:test", Local: "Union"}, "urn:test", &types.UnionType{
		MemberTypes: []types.QName{int10.QName},
	})
	if err != nil {
		t.Fatalf("NewUnionSimpleType: %v", err)
	}
	union.MemberTypes = []types.Type{int10}
...
func TestSimpleTypeValidateListMembers(t *testing.T) {
	item, err := types.NewBuiltinSimpleType(types.TypeNameInteger)
	if err != nil {
		t.Fatalf("NewBuiltinSimpleType: %v", err)
	}
	list, err := types.NewListSimpleType(types.QName{Namespace: "urn:test", Local: "IntList"}, "urn:test", &types.ListType{
		InlineItemType: item,
	}, nil)
	if err != nil {
