...
func TestValidateListTypesWhitespace(t *testing.T) {
	validatorNames := []string{"IDREFS", "ENTITIES", "NMTOKENS"}
	validatorFns := []func(string) error{validateIDREFS, validateENTITIES, validateNMTOKENS}
	...
	tests := []struct {
		...
		{name: "cr separated", value: "a\rb", valid: true},
		...
		{name: "non-xml vt", value: "a\u000bb", valid: false},
		{name: "non-xml ff", value: "a\u000cb", valid: false},
	}
		...
		t.Run(name, func(t *testing.T) {
			for _, tt := range tests {
				t.Run(tt.name, func(t *testing.T) {
					err := fn(tt.value)
					if (err == nil) != tt.valid {
						t.Fatalf("%s(%q) error=%v, want valid=%v", name, tt.value, err, tt.valid)
					}
				})
			}
		})
...
func TestBuiltinTypeMeasureLength_ListWhitespace(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := listType.MeasureLength(tt.value); got != tt.want {
				t.Fatalf("MeasureLength(%q) = %d, want %d", tt.value, got, tt.want)
			}
		})
	}
}
