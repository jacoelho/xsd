  ...
  <xs:simpleType name="QNameList">
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			gotDefault, err := comp.canonicalizeNormalizedDefault(tc.lexical, normalized, typ, ctx)
			if err != nil {
				t.Fatalf("canonicalize default: %v", err)
			}
			gotGeneral, err := comp.canonicalizeNormalized(tc.lexical, normalized, typ, ctx)
			if err != nil {
