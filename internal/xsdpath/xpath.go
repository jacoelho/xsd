...
func CanonicalizeNodeTest(test NodeTest) NodeTest {
	...
	test.Namespace = xsdlex.NamespaceEmpty
	return test
}
...
func parseNodeTest(token string, nsContext map[string]string, kind nodeTestKind) (NodeTest, error) {
	...
	if before, ok := strings.CutSuffix(token, ":*"); ok {
		...
		if !xsdlex.IsValidNCName(prefix) {
			return NodeTest{}, xpathErrorf("xpath step has invalid prefix %q", token)
		}
		nsURI, ok := xsdlex.ResolveNamespace(prefix, nsContext)
		if !ok {
	...
	if !xsdlex.IsValidQName(token) {
		return NodeTest{}, xpathErrorf("xpath step has invalid QName %q", token)
	}

	prefix, local, hasPrefix := xsdlex.SplitQName(token)
	if hasPrefix {
		nsURI, ok := xsdlex.ResolveNamespace(prefix, nsContext)
		if !ok {
			return NodeTest{}, xpathErrorf("xpath step uses undeclared prefix %q", prefix)
