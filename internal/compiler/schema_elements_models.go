...
func (b *schemaBuilder) buildModels() error {
	...
	for _, entry := range b.registry.TypeOrder {
		...
		case *model.SimpleContent:
			...
			textType, ok := b.complexTypes.SimpleContentType(ct)
			if !ok {
				return fmt.Errorf("runtime build: complex type %s missing simple-content type", entry.QName)
			}
			if textType == nil {
		...
		default:
			...
			if !ok {
				return fmt.Errorf("runtime build: complex type %s missing derived content", entry.QName)
			}
			if particle == nil {
				complexModel.Content = runtime.ContentEmpty
				break
