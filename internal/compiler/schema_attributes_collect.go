...
func (b *schemaBuilder) collectAttrUses(ct *model.ComplexType) ([]runtime.AttrUse, *model.AnyAttribute, error) {
	...
	if b.complexTypes == nil {
		return nil, nil, fmt.Errorf("runtime build: complex types are nil")
	}
	attrs, wildcard, ok := b.complexTypes.AttributeUses(ct)
	if !ok {
		return nil, nil, fmt.Errorf("runtime build: complex type %s missing derived attributes", ct.QName)
	}
	if len(attrs) == 0 {
