...
func TestBuildComplexTypePlanEntriesAndLookup(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("buildComplexTypePlanEntries() error = %v", err)
	}
...
func TestBuildNilRegistry(t *testing.T) {
	if _, err := buildComplexTypePlanEntries(nil, ComplexTypePlanFuncs{}); err == nil {
		t.Fatal("buildComplexTypePlanEntries(nil, ...) expected error")
	}
}
