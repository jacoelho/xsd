...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="uriSpace" type="xs:anyURI" default="http://exa mple.com"/>
	...
	if _, err := Compile(sch, reg); err != nil {
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  <xs:element name="root" type="xs:time" fixed="23:59:60+02:00"/>
	...
	leapValue, err := value.Parse(value.KindTime, []byte("23:59:60+02:00"))
	if err != nil {
		t.Fatalf("parse leap value: %v", err)
	}
	want := runtime.TemporalKeyBytes(nil, 2, leapValue.Time, leapValue.TimezoneKind, leapValue.LeapSecond)
	if !bytes.Equal(got, want) {
		t.Fatalf("fixed key = %x, want %x", got, want)
	}

	nonLeapValue, err := value.Parse(value.KindTime, []byte("22:00:00Z"))
	if err != nil {
		t.Fatalf("parse non-leap value: %v", err)
	}
	nonLeapKey := runtime.TemporalKeyBytes(nil, 2, nonLeapValue.Time, nonLeapValue.TimezoneKind, nonLeapValue.LeapSecond)
