...
func (r *typeResolver) variety(st *types.SimpleType) types.SimpleTypeVariety {
	...
	if st.ResolvedBase != nil {
		...
		if bt := builtinForType(st.ResolvedBase); bt != nil && types.IsBuiltinListTypeName(bt.Name().Local) {
			return types.ListVariety
		}
	}
	if st.Restriction != nil {
		if st.Restriction.SimpleType != nil {
			...
			if bt := builtinForType(st.Restriction.SimpleType); bt != nil && types.IsBuiltinListTypeName(bt.Name().Local) {
				return types.ListVariety
			}
		}
