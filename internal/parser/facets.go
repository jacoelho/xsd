...
func CollectSimpleTypeFacets(schema *Schema, st *model.SimpleType, convert model.DeferredFacetConverter) ([]model.Facet, error) {
	if convert == nil {
...
func CollectRestrictionFacets(schema *Schema, restriction *model.Restriction, baseType model.Type, convert model.DeferredFacetConverter) ([]model.Facet, error) {
	if convert == nil {
		convert = DefaultDeferredFacetConverter
	}
