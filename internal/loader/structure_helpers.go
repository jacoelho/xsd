...
func convertDeferredFacet(df *facets.DeferredFacet, baseType types.Type) (facets.Facet, error) {
	...
	switch df.FacetName {
	case "minInclusive":
		return facets.NewMinInclusive(df.FacetValue, baseType)
	case "maxInclusive":
		return facets.NewMaxInclusive(df.FacetValue, baseType)
	case "minExclusive":
		return facets.NewMinExclusive(df.FacetValue, baseType)
	case "maxExclusive":
		return facets.NewMaxExclusive(df.FacetValue, baseType)
	default:
...
func hasEnumerationFacet(facetList []facets.Facet) bool {
	for _, f := range facetList {
		if _, ok := f.(*facets.Enumeration); ok {
			return true
		}
	}
	return false
