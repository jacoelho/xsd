...
func validateFacetInheritance(derivedFacets []facets.Facet, baseType types.Type) error {
...
func implicitRangeFacetsForBuiltin(bt *types.BuiltinType) []facets.Facet {
	...
	if info.minValue != nil {
		if info.minInclusive {
			if facet, err := facets.NewMinInclusive(*info.minValue, bt); err == nil {
				result = append(result, facet)
			}
		} else if facet, err := facets.NewMinExclusive(*info.minValue, bt); err == nil {
			result = append(result, facet)
		}
	}
	if info.maxValue != nil {
		if info.maxInclusive {
			if facet, err := facets.NewMaxInclusive(*info.maxValue, bt); err == nil {
				result = append(result, facet)
			}
		} else if facet, err := facets.NewMaxExclusive(*info.maxValue, bt); err == nil {
			result = append(result, facet)
		}
	}
	return result
...
func extractRangeFacetInfo(facetsList []facets.Facet) rangeFacetInfo {
	var info rangeFacetInfo
	for _, facet := range facetsList {
		lex, ok := facet.(facets.LexicalFacet)
		if !ok {
			continue
		}
		val := lex.GetLexical()
...
func validateRangeFacetInheritance(derivedFacets, baseFacets []facets.Facet, baseType types.Type) error {
