...
func validateFacetConstraints(facetList []facets.Facet, baseType types.Type, baseQName types.QName) error {
	...
	for _, facet := range facetList {
		...
		case "length":
			if ivf, ok := facet.(facets.IntValueFacet); ok {
				val := ivf.GetIntValue()
				length = &val
			}
...
func validateListItemValue(itemType types.Type, value string) error {
	if st, ok := itemType.(types.SimpleTypeDefinition); ok {
...
func validateEnumerationValues(facetList []facets.Facet, baseType types.Type) error {
	var enumValues []string
	for _, f := range facetList {
		if f.Name() != "enumeration" {
			continue
		}
		if enum, ok := f.(*facets.Enumeration); ok {
