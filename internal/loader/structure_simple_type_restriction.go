...
func validateRestriction(schema *schema.Schema, st *types.SimpleType, restriction *types.Restriction) error {
	...
	// convert facets to []facets.Facet for validation
	// also process deferred facets (range facets that couldn't be constructed during parsing)
	facetList := make([]facets.Facet, 0, len(restriction.Facets))
	var deferredFacets []*facets.DeferredFacet
...
func validateLengthFacetInheritance(derivedFacets []facets.Facet, baseType *types.SimpleType) error {
...
func facetsFromRestriction(restriction *types.Restriction) []facets.Facet {
	if restriction == nil {
		return nil
	}
	result := make([]facets.Facet, 0, len(restriction.Facets))
	for _, f := range restriction.Facets {
		if facet, ok := f.(facets.Facet); ok {
...
func findIntFacet(facetList []facets.Facet, name string) (int, bool) {
	for _, facet := range facetList {
		if facet.Name() != name {
			continue
		}
		if iv, ok := facet.(facets.IntValueFacet); ok {
