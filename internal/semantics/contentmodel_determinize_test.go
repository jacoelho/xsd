...
func TestDeterminizeSequence(t *testing.T) {
	...
	compiled, err := CompileContentModel(glu, matchers, Limits{MaxDFAStates: 16})
	if err != nil {
...
func TestDeterminizeFallbackToNFA(t *testing.T) {
	...
	compiled, err := CompileContentModel(glu, matchers, Limits{MaxDFAStates: 1})
	if err != nil {
		t.Fatalf("Compile: %v", err)
	}
...
func TestDeterminizeWildcardEdges(t *testing.T) {
	...
	compiled, err := CompileContentModel(glu, matchers, Limits{MaxDFAStates: 8})
	if err != nil {
		t.Fatalf("Compile: %v", err)
	}
	if compiled.Kind != runtime.ModelDFA {
...
func TestCompileRejectsNonNullableEmptyLanguage(t *testing.T) {
	...
	compiled, err := CompileContentModel(glu, nil, Limits{})
	if err != nil {
		t.Fatalf("Compile: %v", err)
	}
	if compiled.Kind != runtime.ModelDFA {
