...
func TestResolveUnionAndListItemTypes(t *testing.T) {
	...
	if members := parser.ResolveUnionMemberTypes(unionSchema, unionType); len(members) == 0 {
	...
	item := parser.ResolveListItemType(listSchema, listType)
...
func TestValidateValueAgainstFacets(t *testing.T) {
	...
	collectedFacets, err := parser.CollectSimpleTypeFacets(schema, st, nil)
	if err != nil {
...
func TestResolveSimpleContentBaseType(t *testing.T) {
	...
	textType := parser.ResolveSimpleContentBaseTypeFromContent(schema, sc)
...
func TestTypeDerivationHelpers(t *testing.T) {
	...
	base := model.GetBuiltin(model.TypeNameString)
	if base == nil {
	...
	if !FieldTypesCompatible(derived, base) {
