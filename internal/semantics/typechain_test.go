...
func TestLookupTypeIgnoresTypedNil(t *testing.T) {
	t.Parallel()

	schema := parser.NewSchema()
	name := model.QName{Namespace: "urn:test", Local: "Missing"}

	var missing *model.SimpleType
	schema.TypeDefs[name] = missing

	got, ok := LookupType(schema, name)
	if ok || got != nil {
		t.Fatalf("LookupType() = (%v, %v), want (nil, false)", got, ok)
	}
}
