...
func (c *artifactCompiler) compileAttributeDefaults(registry *analysis.Registry) error {
...
func (c *artifactCompiler) compileAttributeUseDefaultFixed(decl *model.AttributeDecl) error {
	if st, ok := model.AsSimpleType(decl.Type); ok && model.IsPlaceholderSimpleType(st) {
