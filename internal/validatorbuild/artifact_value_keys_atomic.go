...
func (c *artifactCompiler) keyBytesAtomic(normalized string, typ model.Type, ctx map[string]string) (keyBytes, error) {
	...
	kind, bytes, err := runtime.KeyForPrimitiveName(primName, normalized, ctx)
	if err != nil {
		return keyBytes{}, err
	}
