...
func (c *artifactCompiler) validatePartialFacets(normalized string, typ model.Type, facets []model.Facet) error {
	if len(facets) == 0 {
		return nil
	}
...
func (c *artifactCompiler) validateMemberFacets(normalized string, typ model.Type, facets []model.Facet, ctx map[string]string, includeEnum bool) error {
	if len(facets) == 0 {
		return nil
	}
	for _, facet := range facets {
...
func (c *artifactCompiler) validateRangeFacet(normalized string, typ model.Type, facetName, facetLexical string) error {
...
func (c *artifactCompiler) shouldSkipLengthFacet(typ model.Type, facet model.Facet) bool {
