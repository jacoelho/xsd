...
// It is fail-stop and requires a configured resolver for root resolution.
func (l *SchemaLoader) LoadParsed(location string) (*parser.Schema, error) {
...
// It is fail-stop and only requires a resolver when directives need external resolution.
func (l *SchemaLoader) LoadResolvedParsed(doc io.ReadCloser, systemID string) (*parser.Schema, error) {
...
func (l *SchemaLoader) resolvePendingImportsFor(sourceKey loadKey) error {
	...
	for _, entry := range pendingDirectives {
		switch entry.kind {
		case parser.DirectiveInclude:
			l.imports.markMerged(parser.DirectiveInclude, entry.targetKey, sourceKey)
		case parser.DirectiveImport:
			l.imports.markMerged(parser.DirectiveImport, entry.targetKey, sourceKey)
		}
	}
