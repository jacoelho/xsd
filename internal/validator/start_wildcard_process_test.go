...
func TestResolveSymbol(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resolveCalls := 0
			resolved, err := ResolveStartSymbol(tt.pc, tt.sym, func(runtime.SymbolID) bool {
				resolveCalls++
				return tt.resolveOK
			}, func() error {
				return strictErr
			...
			if resolved != tt.wantResolve {
				t.Fatalf("ResolveStartSymbol() resolved = %v, want %v", resolved, tt.wantResolve)
			}
			if tt.wantErr == nil && err != nil {
				t.Fatalf("ResolveStartSymbol() unexpected error: %v", err)
			}
			if tt.wantErr != nil && !errors.Is(err, tt.wantErr) {
				t.Fatalf("ResolveStartSymbol() error = %v, want %v", err, tt.wantErr)
			}
...
func TestResolveSymbolUnknownProcessContents(t *testing.T) {
	_, err := ResolveStartSymbol(runtime.ProcessContents(255), 1, func(runtime.SymbolID) bool {
		return true
	}, nil)
	if err == nil {
