...
// CheckTypeDerivation enforces xsi:type derivation compatibility against the
// declared element type and blocking/final constraints.
func CheckTypeDerivation(rt *runtime.Schema, derived, base runtime.TypeID, block runtime.ElemBlock) error {
	if derived == 0 || base == 0 {
	...
	startIDs, endIDs, okIDs := checkedSpan(off, ln, len(rt.Ancestors.IDs))
	startMasks, endMasks, okMasks := checkedSpan(off, ln, len(rt.Ancestors.Masks))
	if !okIDs || !okMasks {
