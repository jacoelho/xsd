...
func ValidateComplex(
	...
	for i, attr := range input {
		...
		case ClassXSIUnknown:
			return nil, seenID, diag.New(xsderrors.ErrAttributeNotDeclared, "unknown xsi attribute")
		...
		if attr.Sym != 0 {
			...
			if ok {
				if use.Use == runtime.AttrProhibited {
					return nil, seenID, diag.New(xsderrors.ErrAttributeProhibited, "attribute prohibited")
				}
		...
		if ct.AnyAttr == 0 {
			return nil, seenID, diag.New(xsderrors.ErrAttributeNotDeclared, "attribute not declared")
		}
