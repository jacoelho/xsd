...
func ApplyDefaults(
	...
	for i := range uses {
		...
		if use.Use == runtime.AttrRequired {
			return nil, diag.New(xsderrors.ErrRequiredAttributeMissing, "required attribute missing")
		}
		...
		if isIDValidator != nil && isIDValidator(use.Validator) {
			if seenID {
				return nil, diag.New(xsderrors.ErrMultipleIDAttr, "multiple ID attributes on element")
			}
			seenID = true
		}
