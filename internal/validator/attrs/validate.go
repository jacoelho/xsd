...
// Result holds the validated and applied attributes for one type-level validation pass.
type Result struct {
	Attrs   []Start
	Applied []Applied
}
...
func ValidateType(
	...
	if classified.DuplicateErr != nil {
		return Result{}, classified.DuplicateErr
	}
	...
	if typ.Kind != runtime.TypeComplex {
		...
		return Result{Attrs: simpleValidated}, nil
	}

	if int(typ.Complex.ID) >= len(rt.ComplexTypes) {
		return Result{}, fmt.Errorf("complex type %d not found", typ.Complex.ID)
	}
