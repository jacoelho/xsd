...
// Cache stores parsed primitive values and derived facet measurements for one validation.
type Cache struct {
	intVal         num.Int
	decVal         num.Dec
	fractionDigits int
...
func (c *Cache) SetDecimal(val num.Dec) {
	if c == nil {
		return
	}
	c.decVal = val
...
func (c *Cache) SetInteger(val num.Int) {
	if c == nil {
		return
	}
	c.intVal = val
...
func (c *Cache) SetFloat32(val float32, class num.FloatClass) {
	if c == nil {
		return
	}
	c.float32Val = val
...
func (c *Cache) SetFloat64(val float64, class num.FloatClass) {
	if c == nil {
		return
	}
	c.float64Val = val
...
func (c *Cache) SetListLength(count int) {
	c.SetLength(count)
}

// Decimal returns one parsed decimal, reusing cached state when present.
func (c *Cache) Decimal(canonical []byte) (num.Dec, error) {
	if c != nil && c.decSet {
		return c.decVal, nil
	}
...
func (c *Cache) Integer(canonical []byte) (num.Int, error) {
	if c != nil && c.intSet {
		return c.intVal, nil
	}
	val, perr := num.ParseInt(canonical)
	if perr != nil {
		return num.Int{}, diag.Invalid("invalid integer")
	}
	if c != nil {
		c.intVal = val
		c.intSet = true
...
func (c *Cache) Float32(canonical []byte) (float32, num.FloatClass, error) {
	if c != nil && c.float32Set {
		return c.float32Val, c.float32Class, nil
	}
	val, class, perr := num.ParseFloat32(canonical)
	if perr != nil {
		return 0, num.FloatFinite, diag.Invalid("invalid float")
	}
	if c != nil {
		c.float32Val = val
		c.float32Class = class
...
func (c *Cache) Float64(canonical []byte) (float64, num.FloatClass, error) {
	if c != nil && c.float64Set {
		return c.float64Val, c.float64Class, nil
	}
	val, class, perr := num.ParseFloat(canonical, 64)
	if perr != nil {
		return 0, num.FloatFinite, diag.Invalid("invalid double")
	}
	if c != nil {
		c.float64Val = val
		c.float64Class = class
...
func (c *Cache) Length(kind runtime.ValidatorKind, normalized []byte) (int, error) {
	...
	case runtime.VHexBinary:
		...
		if err != nil {
			return 0, diag.Invalid("invalid hexBinary")
		}
		length = len(decoded)
	case runtime.VBase64Binary:
		decoded, err := value.ParseBase64Binary(normalized)
		if err != nil {
			return 0, diag.Invalid("invalid base64Binary")
		}
		length = len(decoded)
	default:
...
func (c *Cache) DigitCounts(kind runtime.ValidatorKind, canonical []byte) (int, int, error) {
	...
	default:
		return 0, 0, diag.Invalid("digits facet not applicable")
	}
	if c != nil {
		c.totalDigits = total
