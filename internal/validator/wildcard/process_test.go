...
func TestResolveSymbol(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			resolveCalls := 0
			resolved, err := ResolveSymbol(tt.pc, tt.sym, func(runtime.SymbolID) bool {
				resolveCalls++
				return tt.resolveOK
			}, func() error {
				return strictErr
			...
			if resolved != tt.wantResolve {
				t.Fatalf("ResolveSymbol() resolved = %v, want %v", resolved, tt.wantResolve)
			}
			if tt.wantErr == nil && err != nil {
				t.Fatalf("ResolveSymbol() unexpected error: %v", err)
			}
			if tt.wantErr != nil && !errors.Is(err, tt.wantErr) {
				t.Fatalf("ResolveSymbol() error = %v, want %v", err, tt.wantErr)
			}
...
func TestResolveSymbolUnknownProcessContents(t *testing.T) {
	_, err := ResolveSymbol(runtime.ProcessContents(255), 1, func(runtime.SymbolID) bool {
		return true
	}, nil)
	if err == nil {
