...
func Benchmark_StepModel_DFA_Exact(b *testing.B) {
	...
	setRuntimeDFAModels(b, fx.schema, make([]runtime.DFAModel, 2))
...
func Benchmark_StepModel_NFA_Exact(b *testing.B) {
	fx := buildModelFixture(b)
	setRuntimeNFAModels(b, fx.schema, make([]runtime.NFAModel, 2))
...
func BenchmarkValidateNFAFrameState(b *testing.B) {
	fx := buildModelFixture(b)
	setRuntimeNFAModels(b, fx.schema, make([]runtime.NFAModel, 2))
	fx.schema.ModelBundle().NFA[1] = buildNFASequence(fx.symA, fx.symB, fx.elemA, fx.elemB)
	ref := runtime.ModelRef{Kind: runtime.ModelNFA, ID: 1}
...
func Benchmark_StepModel_All_Exact(b *testing.B) {
	fx := buildModelFixture(b)
	setRuntimeAllModels(b, fx.schema, make([]runtime.AllModel, 2))
...
func BenchmarkValidateAllFrameState(b *testing.B) {
	fx := buildModelFixture(b)
	setRuntimeAllModels(b, fx.schema, make([]runtime.AllModel, 2))
	fx.schema.ModelBundle().All[1] = runtime.AllModel{
		Members: []runtime.AllMember{
