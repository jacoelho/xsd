...
// to the referencing file. Reader eagerly reads trusted in-memory schemas and
// uses only sources passed to Compile unless callers attach a Resolver with
// SchemaSource.WithResolver. LimitedReader caps untrusted reader input. HTTP
// and network fetches are never performed by default.
//
