...
func TestTypeAncestorsMatchRecursiveWalk(t *testing.T) {
	engine := mustCompile(t, `
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  <xs:simpleType name="small"><xs:restriction base="xs:int"><xs:maxInclusive value="10"/></xs:restriction></xs:simpleType>
  <xs:simpleType name="tiny"><xs:restriction base="small"><xs:maxInclusive value="3"/></xs:restriction></xs:simpleType>
  <xs:simpleType name="mix"><xs:union memberTypes="small xs:string"/></xs:simpleType>
  <xs:simpleType name="mixes"><xs:union memberTypes="mix xs:boolean"/></xs:simpleType>
  <xs:complexType name="base">
    <xs:sequence><xs:element name="a" type="xs:string"/></xs:sequence>
  </xs:complexType>
  <xs:complexType name="ext">
    <xs:complexContent><xs:extension base="base"><xs:sequence><xs:element name="b" type="xs:string" minOccurs="0"/></xs:sequence></xs:extension></xs:complexContent>
  </xs:complexType>
  <xs:complexType name="extra" block="extension">
    <xs:complexContent><xs:extension base="ext"/></xs:complexContent>
  </xs:complexType>
  <xs:complexType name="withSimpleContent">
    <xs:simpleContent><xs:extension base="tiny"><xs:attribute name="u" type="xs:string"/></xs:extension></xs:simpleContent>
  </xs:complexType>
  <xs:element name="root" type="extra"/>
</xs:schema>`)
	rt := engine.rt
	var all []typeID
	for id := range rt.SimpleTypes {
		all = append(all, simpleRef(simpleTypeID(id)))
	}
	for id := range rt.ComplexTypes {
		all = append(all, complexRef(complexTypeID(id)))
	}
	for _, typ := range all {
		for _, base := range all {
			gotMask, gotOK := rt.typeDerivationMask(typ, base)
			wantMask, wantOK := rt.computeTypeDerivationMask(typ, base)
			if gotMask != wantMask || gotOK != wantOK {
				t.Errorf("typeDerivationMask(%s, %s) = (%v, %v), want (%v, %v)",
					rt.typeLabel(typ), rt.typeLabel(base), gotMask, gotOK, wantMask, wantOK)
			}
		}
	}
}

func TestFreezeRejectsTypeAncestorDrift(t *testing.T) {
	const schema = `<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
  ...
  <xs:element name="root" type="derived"/>
	...
	mutations := []struct {
		name   string
		mutate func(ct *complexType)
	}{
		{
			name: "mask drift",
			mutate: func(ct *complexType) {
				ct.Ancestors[0].Mask ^= blockExtension
			},
		},
		{
			name: "dropped ancestors",
			mutate: func(ct *complexType) {
				ct.Ancestors = nil
			},
		},
		{
			name: "extra ancestor",
			mutate: func(ct *complexType) {
				ct.Ancestors = append(ct.Ancestors, ct.Ancestors[0])
			},
		},
	}
	for _, tc := range mutations {
		t.Run(tc.name, func(t *testing.T) {
			...
			id, ok := engine.rt.GlobalTypes[mustQName(t, engine.rt, "derived")].complex()
			if !ok {
				t.Fatal("derived is not a complex type")
			}
			ct := &engine.rt.ComplexTypes[id]
			if len(ct.Ancestors) == 0 {
				t.Fatal("derived has no ancestors")
			}
			tc.mutate(ct)
			err := validateRuntimeSchema(engine.rt)
			expectCategoryCode(t, err, InternalErrorCategory, ErrInternalInvariant)
